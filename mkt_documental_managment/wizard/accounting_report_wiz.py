...
class AccountingReport(models.Model):
    ...
    def _get_file_name(self, function_name, file_name=False):
        dic_name = super(AccountingReport, self)._get_file_name(function_name, file_name=_('Accounting Report'))
        return dic_name
    ...
    def _get_datas_report_xlsx(self, workbook):
        ...
        for line in records:
            ...
            ws.write(row, 5, line['partner'][:15] + ',' + line['document_type_code'] + ' ' + line['document'])
            ws.write(row, 6, line['change_type'] if line['change_type'] else 0)
            ...
            ws.write(row, 10, line['account'])
            ws.write(row, 11, line['annex_code'])
            ws.write(row, 12, line['cost_center'])
            if line['debit'] > 0:
                ws.write(row, 13, 'D')
            if line['credit'] > 0:
                ws.write(row, 13, 'H')
            ...
            ws.write(row, 15, '')
            ws.write(row, 16, '')
            ws.write(row, 17, 'DR' if line['account'] == '421203' else line['document_type_code'])
            # ws.write(row, 18, line['document'])
            ws.write(row, 18, line['detraction_operation_number'] if line['account'] == '421203' else line['document_number'])
            if line['account'] == '421203':
                if line['detraction_payment_date']:
                    ws.write(row, 19, line['detraction_payment_date'], stl4)
                else:
                    ws.write(row, 19, line['due_date'] if line['due_date'] else line['document_date'], stl4)
            else:
                ws.write(row, 19, line['document_date'], stl4)
            # ws.write(row, 19, line['due_date'] if line['account'] == '421203' else line['document_date'], stl4)
            ...
            ws.write(row, 36, '')
            ws.write(row, 37, line['soles_detraction_perception_amount'])
            ws.write(row, 38, '')
            ws.write(row, 39, '')
            ws.write(row, 40, line['tax_igv'])
            row += 1
    ...
    def _get_query(self):
        query = """
            SELECT
                ...
                dsd.documental_settlement_id AS dsd,
                dsd.subdiary AS subdiary,
                dsd.voucher_number AS voucher_number,
                dsd.voucher_date AS voucher_date,
                CASE
                    WHEN dsd.currency = 'soles' THEN 'MN'
                    WHEN dsd.currency = 'dolares' THEN 'US'
                    ELSE ' '
                END AS currency_code,
                dsd.currency AS currency,
                COALESCE(dsd.partner, rp.name) AS partner,
                dsd.document_type AS document_type,
                slt.name AS document_type,
                dsd.document_type_code AS document_type_code,
                dsd.document AS document,
                slj.document_number AS document_number,
                dsd.main_gloss AS main_gloss,
                dsd.change_type AS change_type,
                dsd.conversion_type AS conversion_type,
                dsd.flag_currency_conversion AS flag_currency_conversion,
                dsd.exchange_type_date AS exchange_type_date,
                dsd.detraction_operation_number AS detraction_operation_number,
                dsd.detraction_payment_date AS detraction_payment_date,
                slj.account_id AS account_id,
                slj.detraction_retention_type AS detraction_retention_type,
                slj.soles_detraction_perception_amount as soles_detraction_perception_amount,
                cc.code AS cost_center,
                aa.code AS account,
                CASE
                    WHEN slj.credit > 0 THEN slj.credit
                    ELSE slj.debit
                END AS credit_debit,
                slj.credit AS credit,
                slj.debit AS debit,
                slj.annex_code AS annex_code,
                slj.auxiliar_annex_code AS auxiliar_annex_code,
                slj.reference_document_type AS reference_document_type,
                slj.reference_document_number AS reference_document_number,
                slj.reference_document_date AS reference_document_date,
                slj.rate_type AS rate_type,
                dsd.date AS document_date,
                dsd.due_date AS due_date,
                dsd.exchange_type_date AS exchange_type_date,
                dsd.detail_gloss AS detail_gloss,
                tt.percentage AS tax_igv
            FROM documental_settlements_detail AS dsd
            LEFT JOIN settlement_line_journal AS slj ON slj.settlement_detail_id=dsd.id
            LEFT JOIN account_account AS aa ON aa.id=slj.account_id
            ...
            LEFT JOIN res_partner AS rp ON rp.id=ds.rq_paid_to_id
            LEFT JOIN tax_taxes AS tt ON tt.id=dsd.tax_igv_id
            LEFT JOIN cost_center AS cc ON cc.id=slj.cost_center_id
            WHERE
                dsd.state IN ('waiting_administration_validation','to_settle','settled') AND
                slt.accountable = True
            ORDER BY ds.id DESC, slj.id DESC
        """
