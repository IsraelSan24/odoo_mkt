...
class SpaceBooking(models.Model):
    ...
    name = fields.Char(string='Reservation', required=True, default=lambda self: self._get_reservation_name())
    ...
    duration = fields.Float(string='Duration (Hours)', default=1)
    item_ids = fields.Many2many('space.booking.item', string='Aditional Items')
    notes = fields.Text(string="Notes")
    state = fields.Selection(selection=states, default='draft')


    def _get_reservation_name(self):
        if self.room_name and self.start_datetime:
            formatted_date = fields.Datetime.to_string(self.start_datetime).split(' ')[0]
        return f'Reservation ({self.room_name})({formatted_date})'
