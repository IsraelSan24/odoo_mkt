...
$(document).ready(function(){
  ...
  // ENVIOS DE GESTION DE DOCUMENTOS
  ...
  $("#enviar_gastos").click(function(){
      ...
      var fecha_tabla11=  $("#fecha_tabla11").val()==""?"":$("#fecha_tabla11").val();
      var fecha_tabla12 =  $("#fecha_tabla12").val()==""?"":$("#fecha_tabla12").val();

      var fecha_general = [fecha_tabla1,fecha_tabla2,fecha_tabla3,fecha_tabla4,fecha_tabla5,fecha_tabla6,fecha_tabla7,fecha_tabla8,fecha_tabla9,fecha_tabla10,fecha_tabla11,fecha_tabla12];
      ...
      var monto11= $("#importe11").val()==""? 0:$("#importe11").val();
      var monto12= $("#importe12").val()==""? 0:$("#importe12").val();

      var monto_general = [monto1,monto2,monto3,monto4,monto5,monto6,monto7,monto8,monto9,monto10,monto11,monto12];
      ...
      for (let z = contador5; z < fecha_general.length; z++){
        if (fecha_general[contador5] !=""){
          if (fecha_general[z] == fecha_general[contador5]){
              fechas6 = fecha_general[contador5];
              resultado6 += parseFloat(monto_general[z]);
              resultadofinal6 = resultado6;
      ...
      $("#importe_total_fecha1").val(resultadofinal1.toFixed(2));

      if(resultadofinal2 != 0 ){
        $("#fecha_importe2").val(fechas2);
        $("#importe_total_fecha2").val(resultadofinal2.toFixed(2));
      }

      if(resultadofinal3 != 0 ){
        $("#fecha_importe3").val(fechas3);
        $("#importe_total_fecha3").val(resultadofinal3.toFixed(2));
      }

      if(resultadofinal4 != 0 ){
        $("#fecha_importe4").val(fechas4);
        $("#importe_total_fecha4").val(resultadofinal4.toFixed(2));
      }
      if(resultadofinal5 != 0 ){
        $("#fecha_importe5").val(fechas5);
        $("#importe_total_fecha5").val(resultadofinal5.toFixed(2));
      }
      if(resultadofinal6 != 0 ){
        $("#fecha_importe6").val(fechas6);
        $("#importe_total_fecha6").val(resultadofinal6.toFixed(2));
      }
  ...
  $('#form_gastos').on('change', function() {
    ...
    var resultado = parseFloat(monto1) + parseFloat(monto2) + parseFloat(monto3) + parseFloat(monto4) + parseFloat(monto5) + parseFloat(monto6) + parseFloat(monto7) + parseFloat(monto8) + parseFloat(monto9) + parseFloat(monto10) + parseFloat(monto11) + parseFloat(monto12); 
    let resultado_final = parseFloat(resultado).toFixed(2)
  ...
  //CANCELAR ENVIOS
  $("#cancelar_requerimiento").click(function(){
    $("#correos_requerimientos").val("");
  });

  $("#cancelar_liquidacion").click(function(){
    $("#correos_liquidacion").val("");
  });

  $("#cancelar_gastos").click(function(){
    $("#correos_gastos").val("");
  });

  $("#cancelar_equipos").click(function(){
    $("#correos_equipos").val("");
  });
