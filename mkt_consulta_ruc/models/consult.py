...
def ConsultarContenidoRUC(sesion, urlReferencia, numeroRUC, numeroRandom):
    ...
    # url_deuda_coactiva = f"https://e-consultaruc.sunat.gob.pe/cl-ti-itmrconsruc/jcrS00Alias?accion=consPorRuc&nroRuc={numeroRUC}&numRnd={numeroRandom}&contexto=ti-it&modo=1&tipo=1&tipdoc=&verCaptcha="
    # _logger.info("\n\n\n contenidoHTML_deuda_coactiva: %s \n\n\n", contenidoHTML_deuda_coactiva)
...
# 			tipoRespuesta = 1
# 			mensajeRespuesta = "Se realizó exitosamente la consulta del número de RUC " + numeroRUC
# 	print("\n\nConsultarContenidoRUCINIT")
# 	print("\n%s 			%s\n" %(tipoRespuesta, mensajeRespuesta))
# 	print("ENDConsultarContenidoRUC\n\n")
# 	return [tipoRespuesta, mensajeRespuesta, response.status_code, lCadena]
    ...
    if (contenidoBusqueda == ".:: Pagina de Mensajes ::."):
        ...
        if (contenidoBusqueda == ""):
            ...
            if(len(arrResultado)> 0):
                ...
                arrResultado = ExtraerContenidoEntreTag(contenidoHTML, posicion, nombreInicio, nombreFin, False)
                # A partir de aquí, podemos extraer el nombre o razón social.
                # _logger.info("\n\n\n arrResultado 2:%s \n\n\n", arrResultado)
                posicion = int(arrResultado[0])
                eSunat = 1
            else:
            ...
            if(eSunat == 1):
                ...
                if(len(lCadena) == 0):
                    ...
                    afecto_rus_item = lCadena[2].split()[-1]
                    # _logger.info('\n lCadena 1:  \n %s \n\n', afecto_rus_item)
                    # afecto_rus.append(afecto_rus_item)
                    lCadena = list()
                    nombreInicio = "<tbody>"
                    nombreFin = "</tbody>"
                    posicion = 0
                    ...
                    while(len(arrResultado)>0):
                        ...
                        # lcadena.append(arrResultado[1].strip().replace('\r\n', ' ').replace('\t', ' '))
                        # _logger.info("\n\n\n En busca de Permitidos 1: %s \n\n\n", arrResultado[1].strip().replace('\r\n', ' ').replace('\t', ' '))
                        arrResultado = ExtraerContenidoEntreTag(contenidoHTML, posicion, nombreInicio, nombreFin, False)
                        # _logger.info("\n\n\n En busca de Permitidos 2: %s \n\n\n", arrResultado)
